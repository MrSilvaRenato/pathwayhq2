...
import {
  ...
  CheckCircle, XCircle as XCircleIcon, Layers,
} from 'lucide-react'
...
function SquadRequestsBanner() {
  ...
  async function act(id, action) {
    ...
    try {
      ...
      toast.success(action === 'approve' ? 'Athlete moved to squad' : 'Request declined')
    } catch {
  ...
  return (
    <div className="col-span-full rounded-2xl border-2 border-amber-300 bg-amber-50 p-4 mb-1">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-black text-amber-900">
            {requests.length} pending squad request{requests.length !== 1 ? 's' : ''} — action required
          </h2>
        </div>
        <Link to="/squads" className="text-xs font-semibold text-amber-700 hover:text-amber-900 flex items-center gap-1">
          View squads <ArrowRight className="h-3 w-3" />
        </Link>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {requests.map(r => (
          <div key={r.id} className="flex items-start gap-3 rounded-xl bg-white border border-amber-200 p-3 shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-500">
                → <span className="font-semibold text-slate-700">{r.squad?.name}</span>
              </p>
              {r.reason && (
                <p className="mt-0.5 text-xs text-slate-400 italic line-clamp-1">"{r.reason}"</p>
              )}
            </div>
            <div className="flex flex-col gap-1.5 shrink-0">
              <button
                ...
                disabled={!!acting}
                className="flex items-center gap-1 rounded-lg bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 px-2.5 py-1 text-[11px] font-bold text-white transition-colors"
              >
                {acting === r.id + 'approve'
              ...
              <button
                ...
                disabled={!!acting}
                className="flex items-center gap-1 rounded-lg border border-slate-200 hover:bg-slate-100 disabled:opacity-50 px-2.5 py-1 text-[11px] font-bold text-slate-600 transition-colors"
              >
                {acting === r.id + 'reject'
                  ? <Loader2 className="h-3 w-3 animate-spin" />
