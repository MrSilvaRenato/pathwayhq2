...
import { Zap, MapPin, Users, Trophy, ArrowLeft, ArrowRight } from 'lucide-react'
...
export default function ClubProfile() {
  ...
  useEffect(() => { api.get(`/clubs/public/${slug}`).then(r => setData(r.data)).catch(() => {}) }, [slug])
      ...
      <nav className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/95 backdrop-blur-md">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-6">
          ...
          <div className="hidden md:flex items-center gap-8">
            <Link to="/clubs" className="text-sm font-medium text-slate-400 hover:text-white transition-colors">← All clubs</Link>
      ...
      <section className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="relative mx-auto max-w-5xl px-6 py-16">
          ...
          <div className="flex items-start gap-6">
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-4xl shadow-xl">
              {sportMeta?.emoji ?? '🏅'}
            </div>
            <div>
              <h1 className="text-4xl lg:text-5xl font-black tracking-tight">{club.name}</h1>
              <div className="mt-3 flex flex-wrap items-center gap-4 text-slate-400 text-sm">
                {sportMeta && <span className="text-slate-300 font-medium">{sportMeta.label}</span>}
                {club.city && <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5" />{club.city}{club.state ? `, ${club.state}` : ''}</span>}
                <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{athletes?.length ?? 0} athletes</span>
              </div>
              {sportMeta?.in2032 && (
                <div className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-sm font-semibold text-emerald-400">
                  <Zap className="h-3.5 w-3.5" /> Brisbane 2032 Olympic sport
                </div>
              )}
            </div>
          </div>
          {club.description && <p className="mt-6 text-slate-400 leading-relaxed max-w-2xl">{club.description}</p>}
        </div>
      </section>
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-1 space-y-5">
            <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5 space-y-3">
              <h2 className="text-sm font-bold text-slate-300 uppercase tracking-wide">Club stats</h2>
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-slate-500">FTEM phases</span>
            ...
            {Object.keys(ftemDist).length > 0 && (
              <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
                <h2 className="mb-4 text-sm font-bold text-slate-300 uppercase tracking-wide">Development pathway</h2>
          ...
          <div className="col-span-2 space-y-5">
            {sportMeta?.in2032 && (
              <div className="rounded-2xl bg-gradient-to-r from-emerald-600/20 to-blue-600/20 border border-emerald-500/20 p-6">
                <div className="flex items-center gap-2 mb-2"><Zap className="h-5 w-5 text-emerald-400" /><h2 className="font-bold text-white">Brisbane 2032 Pathway</h2></div>
                <p className="text-sm text-slate-300 leading-relaxed">{club.name} develops athletes in {sportMeta.label} — a Brisbane 2032 Olympic sport.</p>
              </div>
            )}
            {milestones?.length > 0 && (
              <div className="rounded-2xl border border-white/5 bg-white/[0.03] p-5">
                <h2 className="mb-4 flex items-center gap-2 text-sm font-bold text-slate-300 uppercase tracking-wide">
                  ...
                  {milestones.map(m => (
                    <div key={m.id} className="rounded-xl bg-amber-500/5 border border-amber-500/10 p-4">
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500">{new Date(m.achieved_at).toLocaleDateString('en-AU', { month: 'short', year: 'numeric' })}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
              ...
              <p className="text-sm text-slate-400 mb-6">Ask your coach to set up your athlete profile and start tracking your pathway to 2032.</p>
              <Link to="/signup" className="inline-flex items-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-400 px-6 py-3 text-sm font-bold transition-colors shadow-lg shadow-emerald-500/25">
